<script lang="ts">
  ...
  import { loggedIn, setPage } from '../stores/appStore'
  import { getOptions } from '../stores/optionStore'

  let options: [string, string][]
  const unsubscribe = getOptions('rooms').subscribe(r => (options = r as [string, string][]))
...
<div class="sidebar">
  ...
  <a class="link" on:click={handlePage} href="/automations">
    ...
    <div class="text">Automations</div>
  </a>
  <div class="title">Rooms</div>
  {#each options as [id, text], index (id)}
    <a class="link" on:click={handlePage} href={`/${id}`}>
